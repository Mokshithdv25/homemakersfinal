...
import { addProjectPayment, listProjectPayments, removeProjectPayment, updateProjectPayment, uploadProjectDocument } from "../lib/projectWorkspaceApi";
...
export default function ProjectPayments() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setPayments((rows) => [saved, ...rows]); setForm(EMPTY);
    }
  ...
  const editPayment = async (payment) => {
    const title = window.prompt("Payment title", payment.title || "");
    if (title == null) return;
    const amount = window.prompt("Amount in INR", String(payment.amount_inr || ""));
    if (amount == null) return;
    const amountInr = Number(amount);
    if (!title.trim() || !Number.isFinite(amountInr) || amountInr < 0) {
      setError("Enter a payment title and a valid non-negative amount.");
      return;
    }
    try {
      const updated = await updateProjectPayment(projectId, payment.id, { title: title.trim(), amount_inr: amountInr });
      setPayments((rows) => rows.map((row) => row.id === payment.id ? updated : row));
